...
# Cilium に対応するために experimental を使う
...
kubectl apply -f manifests/service-account.yaml -n hands-on -l color=blue
kubectl apply -f manifests/deployment.yaml -n hands-on -l color=blue
kubectl apply -f manifests/service.yaml -n hands-on
kubectl apply -f manifests/gateway.yaml -n hands-on
...
```zsh
kubectl get services,deployments,gateways -n hands-on
```
